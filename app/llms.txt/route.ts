...
export async function GET() {
  try {
    const res = await fetch(`${BACKEND_URL}/llms.txt?siteUrl=${SITE_URL}`, {
      next: { revalidate: 3600 },
    });

    if (!res.ok) throw new Error(`Backend llms.txt responded with ${res.status}`);
    ...
    console.error("❌ llms.txt proxy failed:", err);

    // Fallback in case of server error
    const fallback = `# NoteHub
