...
const NotePageClient = () => {
  ...
  useEffect(() => {
    ...
    //
    // For mermaid blocks we:
    //   1. Hide the <pre> immediately (before async render) — no flash of code
    //   2. Insert a skeleton preview pane so layout is stable
    //   3. Render SVG async and swap the skeleton content when ready
    //
    ...
    document.querySelectorAll<HTMLDivElement>(".pre-wrapper").forEach((wrapper) => {
      ...
      if (isMermaidLang(lang)) {
        ...
        // This prevents the flash of raw code while mermaid loads
    ...
    const renderMermaidPreviews = async () => {
      ...
      for (const { wrapper, previewEl, codeEl } of mermaidTargets) {
        if (cancelled) return;
    ...
    const handleClick = async (e: MouseEvent) => {
      ...
      if (toggleBtn) {
        ...
        // Update active state on buttons — active = bg-secondary text-foreground
  ...
  if (!note?.content.trim()) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-3 text-muted-foreground">
        ...
        <div className="size-16 bg-muted rounded-full flex items-center justify-center"><Inbox /></div>
        <div>No content</div>
      </div>
    );
  }
        ...
        <div className="max-w-3xl w-full mx-auto relative">
          ...
          <div className="py-8 px-4 space-y-6 border-b border-dashed mb-6 sm:mb-12">
            <div className="flex items-center justify-between">
              <Link href={`/${author?.userName}`} className="flex flex-row items-center w-max gap-3">
                <Avatar className="size-12 bg-muted">
                  <AvatarImage className="w-full h-full object-cover m-0!" src={author?.avatar} alt={author?.fullName || "Author"} />
                  <AvatarFallback>{(author?.fullName || "U").charAt(0).toUpperCase()}</AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                  <div className="font-semibold flex gap-4 text-primary! items-center text-sm">
                    <div className="flex gap-2 items-center">
                      <span>{author?.fullName}</span>
                      {author?.role === "admin" && <BadgeIcon className="size-4 text-blue-500" />}
                    </div>
                    {isOwner && (
            ...
            <div className="flex justify-around gap-8">
              ...
              <DateMeta
                ...
                title={note?.contentUpdatedAt ? formatDate(String(note.contentUpdatedAt)) : ""}
              />
            </div>
          </div>
          ...
          <FloatingActionButtons
            ...
            note={note} username={username} collectionSlug={collectionSlug} noteSlug={noteSlug}
            isOwner={isOwner} handleNavigateToEditor={handleNavigateToEditor}
          />
        </div>
...
const FloatingActionButtons = memo<FloatingActionButtonsProps>(({
  ...
  note, username, collectionSlug, noteSlug, isOwner, handleNavigateToEditor,
}) => (
  <div className="flex gap-2 items-center fixed bottom-4 right-4">
    ...
    <MemoSharePopover shareLink={`${process.env.NEXT_PUBLIC_BASE_URL}/${username}/${collectionSlug}/${noteSlug}`} />
    {isOwner && (
