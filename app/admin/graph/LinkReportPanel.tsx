...
const NoInComming = ({className, size=30} : {className ?: string; size ?: number}) => (
...
const NoOutgoing = ({className, size=30} : {className ?: string; size ?: number}) => (
<svg width={size} height={size} className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export function LinkReportPanel({ crawl }: { crawl: ILinkGraphCrawl }) {
  ...
  const isolatedNodes =
    crawl.nodes?.filter((n) => n.isOrphan && n.isDeadEnd) ?? [];
