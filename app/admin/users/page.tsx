...
export default function UserManagementPage() {
  ...
  const isConfirmValid = () => {
    if (confirmDialog.action === "delete")
      return confirmInput.toLowerCase() === "delete";
    if (confirmDialog.action === "ban")
      return confirmInput.toLowerCase() === "ban";
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Manage your platform users, assign roles, and handle bans.
            </p>
          </div>
          <CreateUserDialog />
        </div>
                      ...
                      >
                        <TableCell className="text-center">
                          <Checkbox
                            checked={isChecked}
                            onCheckedChange={() =>
                              toggleSelectUser(user._id as string)
                            }
                        ...
                        <TableCell className="max-w-60 text-muted-foreground truncate">
                          {user.email}
                        </TableCell>
          ...
          {isLoadingUsers && users.length === 0
            ...
            : users.map((user, index) => {
                ...
                return (
                  <div
                    key={user._id}
                    className={`flex items-center gap-3 px-1 py-3 ${index !== users.length - 1 ? "border-b" : ""} ${isChecked ? "bg-muted/30" : ""}`}
                  >
                    <Checkbox
                      checked={isChecked}
                      onCheckedChange={() =>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-1 font-medium text-[10px] text-muted-foreground truncate">
                          ...
                          {user.collectionsCount || 0} Collections • Joined{" "}
                          {user.createdAt
                            ? new Date(user.createdAt).toLocaleDateString()
                            : "N/A"}
                        </p>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to perform a batch action (
              <b>{confirmDialog.action}</b>) on <b>{selectedIds.length}</b>{" "}
              user(s).
              {confirmDialog.action === "delete" &&
                " This action will soft delete the selected accounts. Their data will remain in the database."}
              {confirmDialog.action === "ban" &&
                " This will prevent the selected users from logging in."}
            </AlertDialogDescription>
          </AlertDialogHeader>
