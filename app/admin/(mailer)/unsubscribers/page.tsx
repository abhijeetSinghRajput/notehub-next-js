...
export default function SuppressionListPage() {
  ...
  const [meta, setMeta] = useState<PaginationMeta>({ total: 0, page: 1, limit: 50 });
  ...
  const fetchData = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      const res = await axiosInstance.get("/mailer/suppressed-emails", { params });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const emails = data.filter((r) => selectedIds.includes(r._id)).map((r) => r.email);
      await axiosInstance.delete("/mailer/suppressed-emails", { data: { emails } });
  ...
  const handleSingleDelete = async (email: string) => {
    try {
      await axiosInstance.delete("/mailer/suppressed-emails", { data: { email } });
  ...
  return (
    <>
      <h1 className="sr-only">Suppression List</h1>
      <Card className="relative bg-background sm:bg-card shadow-none sm:shadow-md sm:border border-none">
        <CardHeader className="flex flex-row justify-between items-center p-0 sm:p-6 pb-6">
          <div>
            ...
            <CardDescription>Emails that have unsubscribed and will not receive future campaigns.</CardDescription>
          </div>
        </CardHeader>
        <CardContent className="space-y-4 p-0 sm:p-6 sm:pt-0">
              ...
              <TableBody>
                {isLoading && data.length === 0 ? (
                  ...
                  data.map((row) => {
                    ...
                    return (
                      <TableRow key={row._id} className={isChecked ? "bg-muted/50" : ""}>
                        ...
                        <TableCell className="max-w-60 font-mono text-sm truncate">{row.email}</TableCell>
                        <TableCell className="text-muted-foreground">{row.campaignId ?? "—"}</TableCell>
                ...
                {!isLoading && data.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="h-20 text-muted-foreground text-center">
                ...
                {isRefreshing && data.length > 0 && (
                  <TableRow>
                    <TableCell colSpan={5} className="bg-muted/30 h-10 text-muted-foreground text-xs text-center">
          ...
          <div className="sm:hidden flex flex-col">
            ...
            {isLoading && data.length === 0 ? (
              ...
              data.map((row, index) => {
                ...
                return (
                  ...
                  >
                    <Checkbox checked={isChecked} onCheckedChange={() => toggleSelect(row._id)} />
                    <div className="flex-1 min-w-0">
                      <p className="font-mono text-sm truncate">{row.email}</p>
                      <p className="mt-0.5 text-muted-foreground text-xs truncate">
                        {row.campaignId ?? "No campaign"} • {new Date(row.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <Button
                      variant="ghost"
            ...
            {!isLoading && data.length === 0 && (
              <div className="py-10 text-muted-foreground text-sm text-center">No suppressed emails found.</div>
            )}

            {isRefreshing && data.length > 0 && (
              <div className="py-4 text-muted-foreground text-xs text-center animate-pulse">Refreshing list...</div>
            )}
          </div>
            ...
            <Pagination className="justify-end mx-0 w-auto">
              <PaginationContent>
                <PaginationItem>
                  <PaginationPrevious
                    href="#"
                    onClick={(e) => { e.preventDefault(); if (hasPreviousPage && !isLoading) setCurrentPage((p) => Math.max(p - 1, 1)); }}
                ...
                <PaginationItem>
                  <PaginationNext
                    href="#"
                    onClick={(e) => { e.preventDefault(); if (hasNextPage && !isLoading) setCurrentPage((p) => p + 1); }}
          ...
          {selectedIds.length > 0 && (
            <div className="bottom-0 slide-in-from-bottom-2 z-10 sticky flex flex-wrap items-center gap-2 bg-card shadow-sm p-3 border rounded-md transition-all animate-in fade-in">
              ...
              <Button variant="destructive" size="sm" onClick={() => { setConfirmOpen(true); setConfirmInput(""); }} className="h-8">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You are about to remove <b>{selectedIds.length}</b> email(s) from the suppression list. They will be eligible to receive future campaigns again.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="my-4">
            <p className="mb-2 text-sm">Please type <strong>delete</strong> to confirm.</p>
            <Input
              value={confirmInput}
